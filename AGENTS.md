...
- 仓库是 uv workspace：根 `pyproject.toml` 是虚拟 workspace root（只声明 `[tool.uv.workspace]`、dev 依赖组与 pytest 配置，本身不是包），packages 分 core 与 app 两部分，各自有 `pyproject.toml`/`src`/`tests`：
  ...
  - `packages/wy-realtime-agent/`（发行名 `wy-realtime-agent`，import 名 `wy_realtime_agent`）：基于 wy-core 的实时语音 agent 应用，由 Qwen-Audio 实时语音大模型经 WebSocket 全双工协议驱动（协议参考包内 `realtime_llm_ws.md`）——`config`（`[realtime]` 段 + `[[mcp.servers]]`）、`protocol`（`RealtimeClient` WS 传输层 + `build_session_config`）、`audio`（`MicSource`/`SpeakerSink` 流式录音与播放，16k 入 / 24k 出）、`agent`（`RealtimeAgent` 事件编排：打断、回声抑制、收集式 function calling）、`mcp` 与 `tools/`（read 内置工具 + `MCPTool`，均为 wy-coding-agent 同名实现的复刻件）、`factory`（`bootstrap` + `create_agent`）与 `main`（console script `wy-realtime-agent`）。实时协议是服务端维护上下文的推送式流，不使用 `wy_core.Model`/`Agent`/`Session`，只复用 `Tool` 契约、`AuditLog` 与 `ToolCall`/`ToolResult` 事件词汇。运行依赖 `wy-core`、`websockets`、`sounddevice`（PyPI wheel 自带 PortAudio）、`mcp`。
