...
复刻自 wy-coding-agent 的 ``mcp.py``(仅改 import 路径),两侧修改需互相
评估。The MCP SDK is fully async and its transport/session objects are
task-scoped async context managers, while the harness ``Tool`` contract is
synchronous. ``MCPClientManager`` therefore runs a dedicated background
thread with its own event loop; each server connection lives inside one
long-lived task on that loop. This module owns only the bridging —
connection lifecycle, tool naming and discovery; per-call execution lives in
``wy_realtime_agent.tools.mcp_tool.MCPTool``, which reaches the loop through
...
from wy_realtime_agent.config import MCPServerConfig
from wy_realtime_agent.tools.mcp_tool import MCPTool
