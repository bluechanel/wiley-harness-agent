...
复刻自 wy-coding-agent 的 ``tools/mcp_tool.py``,两侧修改需互相评估。
本模块只负责"执行一次调用":同步 ``execute`` 经
``asyncio.run_coroutine_threadsafe`` 桥进 MCP 客户端的后台事件循环并等待
(120s 超时),再把 CallToolResult 翻译成结果文本(server 侧 isError →
raise,由调用侧统一转 ``Error: ...``)。连接生命周期(线程/事件循环/会话)
归 ``wy_realtime_agent.mcp`` 的桥接层:它实现 ``acquire()`` 并在构造
``MCPTool`` 时注入自己;本模块不 import MCP SDK。
"""
...
def result_to_text(result: Any) -> str:
    """Flatten a CallToolResult's content blocks into one result string."""
    parts: list[str] = []
