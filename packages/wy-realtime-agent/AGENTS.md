...
- 复用 wy-core 的：realtime 契约全家（`RealtimeModel` + 类型化事件词汇、`RealtimeAgent` + `RealtimeEvent`、`AudioSource`/`AudioSink`、`RealtimeError`）、`Tool` 契约（内置 read 工具与 MCP 工具都按它编写）、`AuditLog` 与 `ToolCall`/`ToolResult`。包根 `__init__` 把 `RealtimeAgent`/`RealtimeEvent`/`RealtimeError` 与全部生命周期事件 dataclass（`SessionReady`/`SpeechStarted`/`UserTranscriptDelta`/`SpeechStopped`/`UserTranscript`/`TurnCommitted`/`TurnDiscarded`/`ResponseStarted`/`AudioDelta`/`AssistantTranscriptDelta`/`AssistantTranscript`/`ResponseDone`/`ToolResultsSubmitted`/`Interrupted`/`ErrorEvent`/`SessionEnded`）从 wy_core re-export 保持兼容（`ToolCall`/`ToolResult` 仍从 wy_core 导入）。不做本地会话镜像/SessionStore（服务端持上下文，审计日志已完整留痕）。
...
- `qwen.py`：`QwenRealtimeModel`（`wy_core.RealtimeModel` 实现），组合 `RealtimeClient`（`client` 可注入替身），`name = config.model`。发送侧契约方法 → wire 事件：`send_user_text` → `conversation.item.create`（`role=user` + `input_text`）、`send_tool_result` → `conversation.item.create`（`function_call_output`）、`update_session` 用 `build_session_config` 组装并**返回实发载荷**（供 core 审计）、其余直通。接收侧 `_translate` 一个 wire dict → 一个类型化事件：`session.updated` → `SessionReady`（session.created 不透出）、`speech_started/stopped` → `SpeechStarted`/`SpeechStopped(reason)`（smart_turn 无效轮带 `reason="turn_invalid"`）、`input_audio_buffer.committed` → `TurnCommitted`、ASR `transcription.delta/completed/failed` → `UserTranscriptDelta(text, stash)`/`UserTranscript`/`ErrorEvent`、base64 解码 `AudioDelta`、字幕 `audio_transcript.delta/done` 与纯文本模态 `text.delta/done` → `AssistantTranscriptDelta`/`AssistantTranscript`、arguments JSON 宽松解析（残缺/非对象回退空 dict）、`conversation.item.ambient_audio_transcription.completed` → `TurnDiscarded`（smart_turn 判非轮次，ambient 转写不透出）；词汇之外的 wire 事件（session.created、output_item/content_part 级、function_call_arguments.delta、voiceprint 注册等）返回 None 静默忽略。
...
- `tui.py`：Textual 流式字幕前端（纯展示层，与 wy-coding-agent 的 tui 同约定：不经包根导出，由 main 组装）。`RealtimeApp(agent)` 后台 worker 消费 `agent.run()`：转写事件维护 `[你]`/`[AI]`"当前行"（增量原位刷新、完成定格、打断在行尾留标记，`user_text`/`assistant_text`/`status_text` 等渲染辅助为可测纯函数）；生命周期事件只驱动状态徽章（`_STATES`：连接中/空闲/**听**/**思考**/**说**/**工具执行**/已结束，彩色圆点 + 弱化补充信息如工具名、结束原因），不产生行；工具调用/结果、`ErrorEvent` 与会话结束以弱化行展示；`AudioDelta` 等词汇之外事件落 match 空档静默忽略。退出绑定 q/Ctrl+C；资源释放留给宿主 finally。
- `main.py`：唯一控制台入口：argparse → `bootstrap()` → 默认启动 `tui.RealtimeApp`；`--plain` 走原控制台逐行打印（只打印完成级事件：`[你]`/`[AI]`/`[工具]`/打断/会话结束，增量与生命周期事件静默忽略）→ KeyboardInterrupt 优雅退出 → finally `close()`；事件类型从 `wy_core` 导入。
...
- 响应结构级 wire 事件透出：`response.output_item.added/done`、`response.content_part.added/done`、`response.function_call_arguments.delta`（工具调用以 arguments.done 聚合为 `FunctionCall`，参数流式增量不透出）。
...
- **编排语义用例在 wy-core**（`tests/test_core_realtime_agent.py`，`FakeRealtimeModel` 类型化事件驱动）；本包 `test_realtime_qwen.py` 只测翻译层（收发两个方向的 wire ↔ 类型化映射、宽松 arguments 解析、未知事件忽略）与少量全链路冒烟；`test_realtime_tui.py` 测 TUI 展示层——渲染辅助直接断言 rich Text，App 行为用 Textual headless `run_test` 全链路冒烟（外层 `asyncio.run` 包装，断言对话区各行 `content.plain`）。改编排行为去 core 加用例，改 wire 翻译在本包加用例。
