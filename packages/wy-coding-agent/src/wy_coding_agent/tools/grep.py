...
def execute_grep(arguments: Mapping[str, Any]) -> str:
    """Execute a grep request and return the formatted search result."""
    ...
    show_numbers = _bool_arg(arguments, "-n", True)
    head_limit = _int_arg(arguments, "head_limit", None)
    offset = _int_arg(arguments, "offset", 0) or 0
    ...
    if _bool_arg(arguments, "multiline", False):
        args.extend(["-U", "--multiline-dotall"])
    if _bool_arg(arguments, "-i", False):
        args.append("-i")
    ...
    if output_mode == "content":
        ...
        context = _int_arg(arguments, "context", None)
        if context is None:
            context = _int_arg(arguments, "-C", None)
        if context is not None:
            args.extend(["-C", str(context)])
        else:
            before = _int_arg(arguments, "-B", None)
            after = _int_arg(arguments, "-A", None)
...
GREP = FunctionTool(definition=GREP_TOOL, execute=execute_grep)
