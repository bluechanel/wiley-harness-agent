...
from wy_coding_agent.tools.grep import GREP, GrepToolError
...
def test_files_with_matches_sorts_by_mtime(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "needle"})
...
def test_content_mode_shows_line_numbers_and_context(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "two", "output_mode": "content", "-C": 1})
...
def test_content_mode_separates_context_groups(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "match", "output_mode": "content", "-A": 1})
...
def test_content_mode_without_line_numbers(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "hello", "output_mode": "content", "-n": False})
...
def test_count_mode_totals(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "x", "output_mode": "count"})
...
def test_case_insensitive_flag(workspace: Path) -> None:
    ...
    assert GREP.execute({"pattern": "hello"}) == "No files found"
    assert GREP.execute({"pattern": "hello", "-i": True}) == "Found 1 file\na.txt"
...
def test_glob_filter_with_brace_expansion(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "needle", "glob": "*.{md,txt}"})
...
def test_negated_glob_excludes_files(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "needle", "glob": "*.py !*_test.py"})
...
def test_type_filter(workspace: Path) -> None:
    ...
    assert GREP.execute({"pattern": "needle", "type": "py"}) == "Found 1 file\na.py"

    with pytest.raises(GrepToolError, match="file type"):
        GREP.execute({"pattern": "needle", "type": "nope"})
...
def test_head_limit_and_offset_pagination(workspace: Path) -> None:
    ...
    limited = GREP.execute({"pattern": "needle", "head_limit": 2})
    assert limited == "Found 2 files limit: 2\nf4.txt\nf3.txt"

    tail = GREP.execute({"pattern": "needle", "head_limit": 2, "offset": 4})
...
def test_content_mode_pagination_note(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "x", "output_mode": "content", "head_limit": 2})
...
def test_no_matches(workspace: Path) -> None:
    ...
    assert GREP.execute({"pattern": "zzz"}) == "No files found"
    assert (
        GREP.execute({"pattern": "zzz", "output_mode": "content"}) == "No matches found"
    )
...
def test_vcs_directories_are_excluded(workspace: Path) -> None:
    ...
    assert GREP.execute({"pattern": "needle"}) == "Found 1 file\nreal.txt"
...
def test_multiline_mode_spans_lines(workspace: Path) -> None:
    ...
    assert GREP.execute({"pattern": "foo.bar"}) == "No files found"
    assert (
        GREP.execute({"pattern": "foo.bar", "multiline": True, "output_mode": "content"})
        == "a.txt:1:foo\na.txt:2:bar"
    )
...
def test_explicit_single_file_path(workspace: Path) -> None:
    ...
    result = GREP.execute({"pattern": "needle", "path": "a.txt", "output_mode": "content"})
