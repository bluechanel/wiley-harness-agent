"""应用层测试辅助:脚本化假模型与简单工具(wy-core 契约)。"""

from __future__ import annotations

import asyncio

from wy_core import Block, Message, Model, ModelEnd, ModelEvent, TextBlock, Tool, Usage
...
def drain(service, user_input: str = "hi") -> list:
    """同步跑完一个回合并收集全部事件(仓库测试不引入 pytest-asyncio)。"""

    async def go() -> list:
        return [event async for event in service.stream(user_input)]
